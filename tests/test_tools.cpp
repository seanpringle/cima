...
TEST_CASE("write_file_lines replace middle lines", "[tools][write_file_lines]") {
    auto sd = make_temp_dir();
    ToolRegistry reg;
    reg.add_defaults(sd, Config{});

    {
        std::ofstream f(sd + "/file.txt");
        f << "line1\nline2\nline3\nline4\nline5\n";
    }

    auto result = reg.execute("write_file_lines",
        R"({"path": "file.txt", "start_line": 2, "end_line": 4, "replace": "REPLACED"})");
    REQUIRE(result);
    CHECK(result->find("ok") != std::string::npos);
    CHECK(result->find("bytes ->") != std::string::npos);

    std::ifstream f(sd + "/file.txt");
    std::string content((std::istreambuf_iterator<char>(f)),
                        std::istreambuf_iterator<char>());
    CHECK(content == "line1\nREPLACED\nline5\n");
...
TEST_CASE("write_file_lines replace first lines", "[tools][write_file_lines]") {
    auto sd = make_temp_dir();
    ToolRegistry reg;
    reg.add_defaults(sd, Config{});
    ...
    auto result = reg.execute("write_file_lines",
        R"({"path": "file.txt", "start_line": 1, "end_line": 2, "replace": "TOP"})");
    REQUIRE(result);
...
TEST_CASE("write_file_lines replace last lines", "[tools][write_file_lines]") {
    auto sd = make_temp_dir();
    ToolRegistry reg;
    reg.add_defaults(sd, Config{});
    ...
    auto result = reg.execute("write_file_lines",
        R"({"path": "file.txt", "start_line": 2, "end_line": 3, "replace": "BOTTOM"})");
    REQUIRE(result);
...
TEST_CASE("write_file_lines replace entire file", "[tools][write_file_lines]") {
    auto sd = make_temp_dir();
    ToolRegistry reg;
    reg.add_defaults(sd, Config{});
    ...
    auto result = reg.execute("write_file_lines",
        R"({"path": "file.txt", "start_line": 1, "end_line": 3, "replace": "ENTIRELY NEW"})");
    REQUIRE(result);

    std::ifstream f(sd + "/file.txt");
    std::string content((std::istreambuf_iterator<char>(f)),
                        std::istreambuf_iterator<char>());
    CHECK(content == "ENTIRELY NEW\n");

    fs::remove_all(sd);
}

TEST_CASE("write_file_lines replace single line", "[tools][write_file_lines]") {
    auto sd = make_temp_dir();
    ToolRegistry reg;
    reg.add_defaults(sd, Config{});
    ...
    auto result = reg.execute("write_file_lines",
        R"({"path": "file.txt", "start_line": 2, "end_line": 2, "replace": "SINGLE"})");
    REQUIRE(result);
...
TEST_CASE("write_file_lines start_line past EOF", "[tools][write_file_lines]") {
    auto sd = make_temp_dir();
    ToolRegistry reg;
    reg.add_defaults(sd, Config{});
    ...
    auto result = reg.execute("write_file_lines",
        R"({"path": "file.txt", "start_line": 10, "end_line": 10, "replace": "APPENDED"})");
    REQUIRE(result);
...
TEST_CASE("write_file_lines end_line past EOF", "[tools][write_file_lines]") {
    auto sd = make_temp_dir();
    ToolRegistry reg;
    reg.add_defaults(sd, Config{});
    ...
    auto result = reg.execute("write_file_lines",
        R"({"path": "file.txt", "start_line": 2, "end_line": 10, "replace": "REPL"})");
    REQUIRE(result);
...
TEST_CASE("write_file_lines start_line < 1", "[tools][write_file_lines]") {
    auto sd = make_temp_dir();
    ToolRegistry reg;
    reg.add_defaults(sd, Config{});

    std::ofstream(sd + "/file.txt") << "content\n";

    auto result = reg.execute("write_file_lines",
        R"({"path": "file.txt", "start_line": 0, "end_line": 2, "replace": "x"})");
    REQUIRE_FALSE(result);
    CHECK(result.error().find("start_line") != std::string::npos);
...
TEST_CASE("write_file_lines end_line < start_line", "[tools][write_file_lines]") {
    auto sd = make_temp_dir();
    ToolRegistry reg;
    reg.add_defaults(sd, Config{});
    ...
    auto result = reg.execute("write_file_lines",
        R"({"path": "file.txt", "start_line": 3, "end_line": 2, "replace": "x"})");
    REQUIRE_FALSE(result);
    CHECK(result.error().find("end_line") != std::string::npos);
...
TEST_CASE("write_file_lines file not found", "[tools][write_file_lines]") {
    auto sd = make_temp_dir();
    ToolRegistry reg;
    reg.add_defaults(sd, Config{});

    auto result = reg.execute("write_file_lines",
        R"({"path": "nonexistent.txt", "start_line": 1, "end_line": 1, "replace": "x"})");
    REQUIRE_FALSE(result);
...
TEST_CASE("write_file_lines path traversal rejected", "[tools][write_file_lines]") {
    auto sd = make_temp_dir();
    ToolRegistry reg;
    reg.add_defaults(sd, Config{});

    auto result = reg.execute("write_file_lines",
        R"({"path": "../../etc/evil", "start_line": 1, "end_line": 1, "replace": "x"})");
    REQUIRE_FALSE(result);

    fs::remove_all(sd);
}

TEST_CASE("write_file_lines preserve trailing newline", "[tools][write_file_lines]") {
    auto sd = make_temp_dir();
    ToolRegistry reg;
    reg.add_defaults(sd, Config{});
    ...
    auto result = reg.execute("write_file_lines",
        R"({"path": "file.txt", "start_line": 2, "end_line": 2, "replace": "new2"})");
    REQUIRE(result);

    std::ifstream f(sd + "/file.txt");
    std::string content((std::istreambuf_iterator<char>(f)),
                        std::istreambuf_iterator<char>());
    // Should still have trailing newline
    CHECK(content == "line1\nnew2\nline3\n");

    fs::remove_all(sd);
}

TEST_CASE("write_file_lines preserve no trailing newline", "[tools][write_file_lines]") {
    auto sd = make_temp_dir();
    ToolRegistry reg;
    reg.add_defaults(sd, Config{});
...
TEST_CASE("write_file_lines file with blank lines", "[tools][write_file_lines]") {
    auto sd = make_temp_dir();
    ToolRegistry reg;
    reg.add_defaults(sd, Config{});
    ...
    auto result = reg.execute("write_file_lines",
        R"({"path": "file.txt", "start_line": 2, "end_line": 3, "replace": "C"})");
    REQUIRE(result);
...
TEST_CASE("write_file_lines multi-line replacement", "[tools][write_file_lines]") {
    auto sd = make_temp_dir();
    ToolRegistry reg;
    reg.add_defaults(sd, Config{});
    ...
    auto result = reg.execute("write_file_lines",
        R"({"path": "file.txt", "start_line": 2, "end_line": 2, "replace": "new2a\nnew2b"})");
    REQUIRE(result);
...
TEST_CASE("write_file_lines empty replacement (delete lines)", "[tools][write_file_lines]") {
    auto sd = make_temp_dir();
    ToolRegistry reg;
    reg.add_defaults(sd, Config{});
    ...
    auto result = reg.execute("write_file_lines",
        R"({"path": "file.txt", "start_line": 2, "end_line": 3, "replace": ""})");
    REQUIRE(result);
...
TEST_CASE("write_file_lines empty file", "[tools][write_file_lines]") {
    auto sd = make_temp_dir();
    ToolRegistry reg;
    reg.add_defaults(sd, Config{});
    ...
    auto result = reg.execute("write_file_lines",
        R"({"path": "file.txt", "start_line": 1, "end_line": 1, "replace": "FIRST"})");
    REQUIRE(result);
