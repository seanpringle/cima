...
TEST_CASE("lua basic execution returns strings and numbers", "[tools][lua]") {
    auto lua = std::shared_ptr<lua_State>(luaL_newstate(), lua_close);
    luaL_openlibs(lua.get());
    auto mutex = std::make_shared<std::mutex>();
    ToolRegistry reg;
    reg.add(make_lua_tool(lua, mutex, 10));

    auto result = reg.execute("lua", R"({"eval": "return 'hello', 42"})");
    REQUIRE(result);
...
TEST_CASE("lua single return value", "[tools][lua]") {
    auto lua = std::shared_ptr<lua_State>(luaL_newstate(), lua_close);
    luaL_openlibs(lua.get());
    auto mutex = std::make_shared<std::mutex>();
    ToolRegistry reg;
    ...
    auto result = reg.execute("lua", R"({"eval": "return 3.14"})");
    REQUIRE(result);
    json j = json::parse(*result);
    REQUIRE(j.is_array());
    REQUIRE(j.size() == 1);
...
TEST_CASE("lua no return values", "[tools][lua]") {
    auto lua = std::shared_ptr<lua_State>(luaL_newstate(), lua_close);
    luaL_openlibs(lua.get());
    auto mutex = std::make_shared<std::mutex>();
    ToolRegistry reg;
    ...
    auto result = reg.execute("lua", R"({"eval": "x = 1 + 2"})");
    REQUIRE(result);
    json j = json::parse(*result);
    REQUIRE(j.is_array());
    CHECK(j.empty());
...
TEST_CASE("lua syntax error", "[tools][lua]") {
    auto lua = std::shared_ptr<lua_State>(luaL_newstate(), lua_close);
    luaL_openlibs(lua.get());
    auto mutex = std::make_shared<std::mutex>();
    ToolRegistry reg;
    ...
    auto result = reg.execute("lua", R"({"eval": "invalid syntax !!!"})");
    CHECK_FALSE(result);
    CHECK(result.error().find("Lua error:") != std::string::npos);
    bool found_expected = result.error().find("syntax") != std::string::npos ||
                          result.error().find("unexpected") != std::string::npos;
    CHECK(found_expected);
}

TEST_CASE("lua runtime error", "[tools][lua]") {
    auto lua = std::shared_ptr<lua_State>(luaL_newstate(), lua_close);
    luaL_openlibs(lua.get());
    auto mutex = std::make_shared<std::mutex>();
    ToolRegistry reg;
    ...
    auto result = reg.execute("lua", R"str({"eval": "error('oops')"})str");
    CHECK_FALSE(result);
    CHECK(result.error().find("Lua error:") != std::string::npos);
    CHECK(result.error().find("oops") != std::string::npos);
}

TEST_CASE("lua error recovery — subsequent calls work after error", "[tools][lua]") {
    auto lua = std::shared_ptr<lua_State>(luaL_newstate(), lua_close);
    luaL_openlibs(lua.get());
    auto mutex = std::make_shared<std::mutex>();
    ToolRegistry reg;
    ...
    // First call: error
    auto result = reg.execute("lua", R"str({"eval": "error('boom')"})str");
    CHECK_FALSE(result);
    CHECK(result.error().find("Lua error:") != std::string::npos);

    // Second call: should succeed on a fresh state
    result = reg.execute("lua", R"({"eval": "return 'recovered'"})");
    REQUIRE(result);
    json j = json::parse(*result);
    REQUIRE(j.is_array());
    REQUIRE(j.size() == 1);
...
TEST_CASE("lua state persistence across calls", "[tools][lua]") {
    auto lua = std::shared_ptr<lua_State>(luaL_newstate(), lua_close);
    luaL_openlibs(lua.get());
    auto mutex = std::make_shared<std::mutex>();
    ToolRegistry reg;
    ...
    // Set a global
    auto result = reg.execute("lua", R"({"eval": "counter = 10"})");
    REQUIRE(result);

    // Read it back
    result = reg.execute("lua", R"({"eval": "return counter"})");
    REQUIRE(result);
    json j = json::parse(*result);
    REQUIRE(j.is_array());
    REQUIRE(j.size() == 1);
...
TEST_CASE("lua timeout aborts long-running code", "[tools][lua]") {
    auto lua = std::shared_ptr<lua_State>(luaL_newstate(), lua_close);
    luaL_openlibs(lua.get());
    auto mutex = std::make_shared<std::mutex>();
    ToolRegistry reg;
    // Use a very short timeout (1 second)
    reg.add(make_lua_tool(lua, mutex, 1));

    auto start = std::chrono::steady_clock::now();
    auto result = reg.execute("lua", R"({"eval": "while true do end"})");
    auto elapsed = std::chrono::steady_clock::now() - start;

    CHECK_FALSE(result);
    CHECK(result.error().find("Lua error:") != std::string::npos);
    CHECK(result.error().find("timed out") != std::string::npos);
    // Should complete well under 30 seconds (default timeout)
    CHECK(elapsed < std::chrono::seconds(10));
}

TEST_CASE("lua timeout does not deadlock subsequent calls", "[tools][lua]") {
    auto lua = std::shared_ptr<lua_State>(luaL_newstate(), lua_close);
    luaL_openlibs(lua.get());
    auto mutex = std::make_shared<std::mutex>();
    ToolRegistry reg;
    ...
    // First call: infinite loop (will time out)
    auto result = reg.execute("lua", R"({"eval": "while true do end"})");
    CHECK_FALSE(result);

    // Second call: should not deadlock — must complete within a short time
    auto start = std::chrono::steady_clock::now();
    result = reg.execute("lua", R"({"eval": "return 'ok'"})");
    auto elapsed = std::chrono::steady_clock::now() - start;
...
TEST_CASE("lua tables not allowed", "[tools][lua]") {
    auto lua = std::shared_ptr<lua_State>(luaL_newstate(), lua_close);
    luaL_openlibs(lua.get());
    auto mutex = std::make_shared<std::mutex>();
    ToolRegistry reg;
    ...
    auto result = reg.execute("lua", R"({"eval": "return {1, 2, 3}"})");
    CHECK_FALSE(result);
    CHECK(result.error().find("cannot return tables") != std::string::npos);
}

TEST_CASE("lua missing eval argument", "[tools][lua]") {
    auto lua = std::shared_ptr<lua_State>(luaL_newstate(), lua_close);
    luaL_openlibs(lua.get());
    auto mutex = std::make_shared<std::mutex>();
    ToolRegistry reg;
    ...
    auto result = reg.execute("lua", R"({})");
    CHECK_FALSE(result);
    CHECK(result.error().find("missing or non-string 'eval'") != std::string::npos);
}
