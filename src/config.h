...
struct Config {
    ...
    std::string system_prompt =
        ...
        "entire session. It stores the conversation history itself in three built-in tables "
        "(`messages`, `tool_calls`, `metadata`), and you can also create your own tables.\n"
        "\n"
        ...
        "### Agent Communication\n"
        "\n"
        "You have per-agent inboxes for communicating with other agents.\n"
        "\n"
        " Use `next_message()` to read and action each message in your inbox.\n"
        " Returns {\"sender\": name, \"message\": markdown} or \"no messages\".\n"
        "\n"
        " Use `send_message(recipient, message)` to send a message to another agent.\n"
        " Returns \"delivered\" or \"no such recipient\".\n"
        "\n"
        "If another agent asks you for help via a message, make sure you also respond using\n"
        " a message, and just summarise for the user.\n"
        "\n"
        "If you are asking another agent for help via a message, make sure you include\n"
        " as much detail as necessary in the message body. They have no knowledge of your\n"
        " current task. They cannot see your Plan document!"
        "\n"
        " Explicitly ask for a response if you need one.\n"
        "\n"
        " Do not poll for a response! It may take the other agent time to action your\n"
        " message. You will receive a push-notification injected into your conversation\n"
        " with the user when you receive a message.\n"
        "\n";
