    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "--help" || arg == "-h") {
            std::cout
                ...
                << "  MODEL               Model name\n"
                << "  LLM_PLANNER_PROMPT  Planner agent system prompt (default: built-in planner prompt)\n"
                << "  LLM_BUILDER_PROMPT  Builder agent system prompt (default: built-in builder prompt)\n"
