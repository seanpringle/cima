        ...
        for (auto& t : tabs) {
            if (!t.is_subagent) {
                t.session->register_call_subagent_tool(
                    /*lookup=*/[&tabs](const std::string& name) -> ChatSession* {
                        for (auto& t2 : tabs) {
                    ...
                    /*is_running=*/[&tabs](const std::string& name) -> bool {
                        for (auto& t2 : tabs) {
                            if (t2.is_subagent && t2.subagent_name == name) {
                    ...
                    /*clear_ui=*/[&tabs](const std::string& name) -> void {
                        for (auto& t2 : tabs) {
                            if (t2.is_subagent && t2.subagent_name == name) {
                                t2.ui_state.entries.clear();
                                t2.ui_state.next_seq = 1;
                                break;
                            }
                        }
                    },
                ...
                for (int ti = 0; ti < (int)tabs.size();) {
                    ...
                    if (BeginTabItem(("   " + tab_label + "   ##tab-" + std::to_string(ti)).c_str(),
                        ...
                        // 40% session tabs (left) + 60% Chat (right) — no menu bar wrapper
                        {
                            ...
                            if (BeginTabBar("##session-tabs")) {
                                ...
                                for (auto& sa_tab : tabs) {
                                    ...
                                    if (BeginTabItem(("   " + sa_tab.title + "   ##sa-" +
                                                         ...
                                                         .c_str())) {
                                        // Render the subagent's config form
                                        render_subagent_tab(sa_tab, cfg, mono_font);
                                        // Then the subagent's chat history underneath
                            ...
                            // Right panel: Chat UI (always the primary agent's conversation)
