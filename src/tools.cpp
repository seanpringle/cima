...
Result<std::string> resolve_path(const std::string& raw_path,
    ...
    auto sd_path = std::filesystem::weakly_canonical(
        std::filesystem::path(safe_dir), ec);
    if (ec) {
...
static Tool make_read_file_tool(const std::string& safe_dir) {
    Tool t;
    t.name = "read_file";
    t.description = "Read the contents of a file (max 200 lines)";
    t.parameters = {
        {"type", "object"},
        {"properties",
         {{"path",
    ...
    t.execute = [safe_dir](const json& args) -> Result<std::string> {
        ...
        if (!file.eof()) {
            result += "...(truncated, >" + std::to_string(max_lines) +
                      " lines)";
        }
        return result;
    };
...
static Tool make_grep_files_tool(const std::string& safe_dir) {
    ...
    t.name = "grep_files";
    t.description =
        "Search file contents using a regex pattern (max 50 results)";
    t.parameters = {
        {"type", "object"},
        {"properties",
         {{"pattern",
...
Result<std::string> ToolRegistry::execute(const std::string& name,
    ...
    try {
        ...
        return std::unexpected("invalid JSON arguments: " +
                               std::string(e.what()));
    }
