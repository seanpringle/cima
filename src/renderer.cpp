...
StreamingMarkdownRenderer::BlockType StreamingMarkdownRenderer::detect_block(
    ...
    // Blank line
    if (trimmed.empty() ||
        std::all_of(trimmed.begin(), trimmed.end(), [](char c) { return c == ' ' || c == '\t'; })) {
    ...
    // Heading: #{1,6} + space + content
    {
        ...
        while (hash_count < 6 && hash_count < (int)sv.size() && sv[hash_count] == '#')
            hash_count++;
        if (hash_count >= 1 && hash_count <= 6 && (int)sv.size() > hash_count &&
            sv[hash_count] == ' ') {
    ...
    // List: leading spaces, then marker, then space, then content
    {
        ...
        if (spaces < sv.size() && std::isdigit(static_cast<unsigned char>(sv[spaces]))) {
            ...
            if (num_end < sv.size() && (sv[num_end] == '.' || sv[num_end] == ')') &&
                num_end + 1 < sv.size() && sv[num_end + 1] == ' ') {
                list_depth = (int)spaces / 2;
                list_ordered = true;
...
void StreamingMarkdownRenderer::process_line(const std::string& line, bool trailing_newline) {
    ...
    if (!list_stack_.empty() && !line.empty() && line[0] == ' ') {
        ...
        std::string content;
        BlockType bt =
            detect_block(line, heading_level, list_depth, list_ordered, list_number, content);
...
void StreamingMarkdownRenderer::emit_block(BlockType type,
    ...
    case BlockType::ListItem: {
        // Manage list stack
        if (!list_stack_.empty() && list_depth == list_stack_.back().depth &&
            list_ordered == list_stack_.back().ordered) {
