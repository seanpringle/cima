...
struct curl_slist* ChatClient::make_headers() const {
    ...
    if (!api_key_.empty()) {
        list = curl_slist_append(
            list, ("Authorization: Bearer " + api_key_).c_str());
    }
    return list;
}
...
size_t ChatClient::write_body(char* ptr, size_t size, size_t nmemb,
                              void* userdata) {
...
size_t ChatClient::write_stream(char* ptr, size_t size, size_t nmemb,
                                void* userdata) {
...
static CURL* setup_curl(const std::string& url, struct curl_slist* headers,
...
Result<void> ChatClient::stream_chat(const json& payload,
